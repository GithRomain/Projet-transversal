...
public class UtilMath
{
    ...
    /*public static List<List<Double>> conversionArrayEnList(double [][] mat)
    {
        List<List<Double>> list = new ArrayList<>();

        for (double[] doubles : mat) {
            list.add(DoubleStream.of(doubles).boxed().collect(Collectors.toList()));
        }
        return list;
    }

    public static double [][] produitMatricielMemeDimensionList(double [][] mat1, double [][] mat2)
    {
        //Renvoie le produit matriciel de mat1 et mat2 qui sont de même dimension
        int n = mat1.length;
        List<List<Double>> list1 = conversionArrayEnList(mat1);
        List<List<Double>> list2 = conversionArrayEnList(mat2);
        double [][] nouvelleMat = new double[n][n];
        List<Double> testList = list1.get(0);
        double test = testList.get(0);
        ...
        for (int i = 0; i < n; i++)
        {
            for (int k = 0; k < n; k++)
            {
                for (int j = 0; j < n; j++)
                {
                    nouvelleMat[i][j] += list1.get(i).get(k) * list2.get(k).get(j);
                }
            }
        }
        return nouvelleMat;
    ...
    public static int [][] conversionMatriceDoubleEnInt(double [][] mat)
    {
        //Renvoie une matrice int, copie de la matrice double
        int n = mat.length;
        int [][] nouvelleMatriceInt = new int[n][n];
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < n; j++)
            {
                nouvelleMatriceInt[i][j] = (int) mat[i][j];
            }
        }
        return nouvelleMatriceInt;
    }
